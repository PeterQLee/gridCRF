...
static int gridCRF_init(boardPy *self, PyObject *args, PyObject *kwds){
  self->V=NULL;
  i32 depth=0;
  ...
  for (i=0;i<dims[0];i++) {
	...
	for (b=-n;b<=0;b++){
	  ...
	  *((*f64)&stack[c++]) = tmp;
      ...
      //TODO: speed up with SSE ops
      l=PyArray_GETPTR(Y,i,j,0);
      p=PyArray_GETPTR(X,i,j,0);
      change[0] = ALPHA * ((*l)-yv[0]) * (*p);
      l=PyArray_GETPTR(Y,i,j,1);
      p=PyArray_GETPTR(X,i,j,1);
...
static void _loopyCPU(gridCRF_py* self, PyArrayObject *X, PyArrayObject *Y){
  ...
  i32 n;
  f32 * F_V = (f32 *) calloc(sizeof(f32) * self->n_factors* 2* dims[0] * dims[1]  *2);
  f32 * V_F = (f32 *) calloc(sizeof(f32) * self->n_factors* 2* dims[0] * dims[1]  *2);
  f32 * marginals= (f32* ) calloc(sizeof(f32)*dims[0]*dims[1]*2);
  f32 * outgoing= (f32*) malloc(sizeof(f32) *  self->n_factors * 2);
  i32 rowv_[4]={0,2,4,6,8,10,12,14};
  const i32 roffset =1 , coffset=2;
  i32 colv_[4]={0,1,4,5,8,9,12,13};
...
static PyArrayObject *predict(gridCRF_py* self, PyObject *args);//PyArrayObject *X, PyArrayObject *Y){
}
