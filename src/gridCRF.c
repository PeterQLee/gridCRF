...
static PyObject* fit (gridCRF_t * self, PyObject *args,PyObject *kwds){
 ...
 //If all these checks have passed the input data is valid.
  ...
  for (i=0;i<n;i++) {
    ...
    if (!PyArray_Check(X) || PyArray_NDIM(X) !=3 ||  PyArray_DIMS(X)[2] != self->n_inp_channels) {
      PyErr_SetString(PyExc_ValueError, "train must only contain 3 dimensional arrays, with the built number of input channels");
      return NULL;
    }
    if (!PyArray_Check(Y) || PyArray_NDIM(Y) !=3 || PyArray_DIMS(Y)[2] != self->n_classes) {
      PyErr_SetString(PyExc_ValueError, "lab must only contain 3 dimensional arrays, of with the specified number of classes");
      return NULL;
    }
  }
...
static PyObject *predict(gridCRF_t* self, PyObject *args, PyObject *kwds){//PyArrayObject *X, PyArrayObject *Y){
  ...
  if (!PyArray_Check(test) || PyArray_NDIM(test) !=3 || PyArray_DIMS(test)[2]!=self->n_inp_channels) {
